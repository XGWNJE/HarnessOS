...
| `development-tool:rust-toolchain` | Rust Toolchain | `development` | 第三方 | 跟踪（winget） | `managed` | `current` | owner-declared | 提供 Rust 编译、包管理、格式化、静态检查和语言服务工具链。 |
| `development-tool:sqlite` | SQLite CLI | `development` | 第三方 | 跟踪（winget） | `managed` | `current` | owner-declared | 提供本地 SQLite 数据库查询、差异与分析命令。 |
...
| `development-tool:winlibs-mingw` | WinLibs MinGW-w64 Toolchain | `development` | 第三方 | 跟踪（winget） | `managed` | `current` | owner-declared | 提供 Windows 原生 GCC/MinGW-w64 编译、链接、调试和配套构建工具链。 |
...
| `software:feishu-desktop` | 飞书桌面客户端 | `communication` | 第三方 | 跟踪（winget） | `managed` | `current` | owner-declared | 在 Windows 工作站提供飞书即时沟通、会议、日历、文档与协作入口。 |
...
| `software:opencode-desktop` | OpenCode Desktop | `ai-agent` | 第三方 | 跟踪（winget） | `managed` | `current` | owner-declared | 提供 OpenCode 的桌面 AI 编程与本地项目交互入口。 |
