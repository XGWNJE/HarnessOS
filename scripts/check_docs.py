...
行数不设硬上限：冗余与否按智能判断，机器只管结构项。
被 hooks/pre-commit 调用（project-doc-boundary skill 的机械锚点）。
只读不写。违规时提示运行 project-doc-boundary skill 修复。
"""
...
# 新增禁止词前先确认它确属 README 的职责边界，避免误伤。
FORBIDDEN = [
    "两行元信息",
    "core.hooksPath",
    "scripts/sync.py",
...
def main() -> int:
    ...
    if problems:
        ...
        for p in problems:
            print(f"  - {p}")
        print("[doc-check] 请运行 project-doc-boundary skill 修复后重新体检。")
        return 1
