...
## 2026-07-24 — 明说规则入口（短通道）

- [框架] notes/ 新增「明说规则」入口：owner 直接声明的意图型规则以 `> 类型：明说规则` 标注；豁免「重复 2 次」门槛，陈化期缩短为 ≥ 2 天（防随口一说固化），合入前加做冲突检查（对照现有 skill 与 global，冲突由 owner 裁决）；来源标注 owner-declared，废止标准为「owner 是否改主意」而非「场景是否失效」
