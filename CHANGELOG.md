...
- [修订] 落点三处，按"哪一环真正做决定"分配：`references/providers.md` 新增「风格倾向：先定哪一系，再定哪个价位」小节，含具体模型映射（Banana 系 → `nano-banana-2`，同时要求最高质量且接受 ￥0.90 才用 `nano-banana-pro-4k-vip`；Image 2.5 系 → 按档位落 `gpt-image-2.5` / `-flare` / `-sunburst`）；`SKILL.md` 的入口模型清单加一条同义规则（选型发生在这里，不能只放在按需参考里）；`generate_image` 的 MCP 工具描述加一句（模型真正看到的选型提示就在工具描述里）。三处措辞一致、指向同一事实。
- 边界说明：①该倾向记为 **owner 声明**，不是可核验的客观结论，也未做对照实验；②只在用户**没点名模型**时生效，用户点名或提出与倾向相反的要求时一律以用户为准；③两系都能做对方擅长的事，此条只决定"默认先试哪个"，不是能力边界；④"扩展像素增加细节"未映射为 Grsai 的某个专用参数——该服务没有独立的放大/外扩参数，按提示词意图表达。
...
- [废止] 原「付费调用有意图确认」与「歧义先澄清，其余直接推进」两条目标删除。前者要求高分辨率、`-vip`/4K 模型与重复生成在调用前取得用户确认；后者列出五种必须先澄清的情况（含比例/分辨率不受支持、要求"最好/最高/4K"且成本可能敏感）。两条都把模型与规格的选择推回用户，与新策略直接冲突。
- [新增] 替代目标两条：「模型与规格：用户声明优先，未声明由 Agent 决定」——含用户两项声明互相冲突时（如点名的模型不支持该比例）在同模型内改选最接近的受支持取值并说明改了什么、不中断流程；「不把选择题推回用户」——只保留"对什么内容出图完全没给"这一种澄清场景，因为出图内容本身无法由 Agent 替用户决定。
- [修订]「成本路由不浪费」改为「成本自控而不请示」：成本控制由"事前请示"改为"默认不选高价通道 + 用户点名时照做"；`-vip`、`-pro-cl`、4K 的使用条件从"用户确认质量优先于成本"改为"用户点名或明确要求质量优先"；未声明档位时默认 standard（`gpt-image-2`，约 ￥0.03~0.06/张）。
- [修订] 同步更新 `scripts/wenje_mcp.py` 中 `generate_image` 的工具描述与 `tier`/`model` 参数说明、`references/providers.md` 的路由规则开头、`SKILL.md` 的入口速览句与退出码 2 的处置文案（参数不受支持时由 Agent 依用户意图改选后继续，不交回用户）。
- [保留] 密钥零暴露、参考图不越权、内容合规、结果可用可见、失败可定性五条目标不变：本次只调整模型与规格的决策归属，不触碰安全与合规边界。
- [修订] `references/architecture.html` 同步本次策略结论：原「两条入口，一个引擎」卡改为「入口与决策归属」并增加一条"用户点名的模型与规格照做，没点名由 Agent 定"。中途试过独立第 4 张卡，在 1440×900 视口触发纵向溢出而放弃，合并回 3 张。重新交付，本节回执取代上一条目中的旧回执：spec `736f82e5…`（3739 B）、artifact `bd2f9b9c…`（809789 B），9 项产物检查全过、组合质量 0 错误 0 警告，`visual-check` 四视口无溢出（1440×900 最小字号 7.05px）。
- 验证：`tests/local_check.py` 14 项全过；`catalog.py check`、`sync.py --check`、`git diff --check` 通过。
...
- [修订] 8 个自有 Skill 的 description 缩短并收窄触发范围，补丁版本各递增一次；`grsai-image-gen`、`image-understand`、`vps-server-info` 将 API 调用细节或动态环境快照移入按需 references，入口分别由 213/114/103 行降至 42/38/37 行，授权、安全红线和完成定义仍常驻。
- [修订] 52 条结构化资产省略 615 个无语义的空字符串与空列表字段；目录渲染内容保持不变，字段说明改为“不适用时省略”。测试夹具同步至当前统一词条模型，修复此前 17 项中 11 项因旧扩展表而失败的基线漂移。
- [废止] 删除未纳入生成与漂移校验链、且与 README Mermaid 重复的 `architecture.html` 和 `architecture.json`；完整结构概览继续由 README 承担，历史文件可从 Git 恢复。

## 2026-09-12 — VPS 换机信息与 SSH 稳定别名刷新

- [修订] `vps-server-info` 1.4.1 → 1.5.0：经 SSH 只读核验，将事实基线切换至 NovixLink LAX 节点；更新系统、内核、内存、swap、磁盘、Nginx 配置、服务入口与退役站点状态，并继续保持连接参数和密钥不进入公开仓库。
- [修订] 本机 SSH 稳定别名 `xgwnje`、`visionguard` 与 `server-infra-primary` 已指向验证可用的新节点；失效的原主机改为明确的 retired 别名，保留审计与回滚入口。

## 2026-09-12 — 统一类型与领域标签命名

- [修订] 分类显示标签统一为中文单概念短名（2~4 字）：类型标签"开发环境工具"→"开发工具"；领域标签"AI 与 Agent"→"AI"、"创意与媒体"→"创作"、"效率办公"→"效率"、"浏览器与 Web"→"浏览器"、"存储与备份"→"存储"、"系统与硬件"→"系统"、"网络与远程"→"网络"。资产 ID 与领域 ID 不变，完整语义保留在 ID 中；README 与根说明的类型提法同步改为中文。
...
- [修订] 项目 `AGENTS.md`：全局规则不再保留文件头版本后，将维护要求由“递增文件头版本并发布”调整为“修改后发布”。
- [修订] `mini-vault` 1.0.0 → 1.0.1、`vps-server-info` 1.4.0 → 1.4.1：删除对已移除全局规则的悬空引用，保留 Skill 自身的授权与重试约束。
- [发布] 全局规则已同步到 Home、Codex、OpenCode、Claude Code 与 DSH；两项 Skill 已同步到现有发布池。
...
- [新增] `development-tool:lark-cli`：登记官方 `@larksuite/cli`，采用 npm 稳定通道；本机已安装并验证 1.0.93 的版本与帮助命令。账号配置与 OAuth 授权不在本次范围。
- [修订] `CATALOG.md` 随两项事实源重建；未扩大范围登记 Node.js，未创建未经指定的工作站 Profile。
...
- [新增] 受管资产增加 `provenance` 与 `upstream_updates`：区分用户自产和第三方内容，并明确是否跟踪外部上游；用户自产项统一显示“上游更新：不适用”，但仍可在仓库内修订和版本化。
...
- [新增] `electron-runtime-debugging` v1.0.0：收拢 Electron 事件类型/运行态差异、IPC 生命周期、ESM 连带影响、打包数据遗漏与全局快捷键自动重复的诊断和验收机制。
- [废止] 2026-08-05 至 2026-08-11 原料评审完成；其余未采纳条目按 owner 决定丢弃，原料文件同步销毁。

## 2026-08-09 — global v1.30.0：瘦身回调（owner-declared）

- [修订] v1.29.0 压缩过度，恢复 v1.28.0 的规则覆盖面：找回范围受控、投入产出、可核验、碰壁止损、验证可视、锁屏即停、Windows 路径、沉淀、建库、分支、提交信息、并行、中转站、安卓工具链等条目，按判断/执行/沟通/协作/安全五组编排；各条文字较 v1.28.0 再精简，保留 owner-declared 与来源标注。
...
## 2026-08-09 — 项目文档职责收拢为单一 Skill（owner-declared）

- [新增] `project-doc-boundary` v1.0.0：只管理项目文档的事实归属与漂移纠正——README、AGENTS.md、CHANGELOG、专题文档和文档地图各有唯一主人；处理重复、错位、过期导航与职责不清。
- [废止] `doc-structure` 与 `init-project`：前者的模板、截图规范和发版文案职责，后者的项目规则初始化与全局规则注入适配，均不再由 Skill 承担；源目录归档，发布池与 `dist/` 历史包清除。
- [修订] 全局规则、pre-commit、文档体检和 observer 改指向新 Skill；全局规则仍由既有发布流水线直接分发，`check_docs.py` 收回为项目文档边界检查，不再检查发版文案格式。
- [修订] `harness-observer` v1.8.0 → v1.10.0：移除对已废止项目初始化 Skill 的提示，并压缩为只记录观察、投递与脱敏的必要机制。
...
- 背景：原定位「备份分发引擎」的「长期约束」只能概括规则/技能；MCP/CLI/配置纳入后规模升级为完整资产管理，原术语退役
- 影响范围：仅表述层变更，无机制变化；体检全绿
...
- [清理] 销毁已加工笔记 `notes/inbox/2026-08-04.md`（该条踩坑已提炼进 global）

## 2026-08-04 — README 表达重构：抽象术语白话化（owner 2026-08-04 指示）

- [修订] `README.md`：抽象概念表达在准确与通俗之间折中——定位句保留「Agent 长期约束的备份分发引擎」并紧跟白话版解释（沉淀规矩和技能文件、统一加工、同步到各 AI 工具）；「冲突检查」「会做错事/不会做某事分流」「pre-commit 硬卡点」等内部黑话改为白话叙述；「单一真相源」「加工单向」保留事实但补通俗说明。无规则语义变化，职责边界体检与发布体检全绿
...
- [修订] HarnessOS 项目 `AGENTS.md`：工作规则「规则来源标注」扩为「规则术语与来源标注」（承接全局迁出的术语定义）；新增「归属收口」条——新声明主题已有 skill 落地机制的直接进该 skill、不再写入全局（防两处漂移，对端评审指出的防再犯缺口）
- [修订] `skills/doc-structure` v1.5.0 → v1.6.0：补命名事实——AGENTS.md 是项目模板通用名，CLAUDE.md 是 Claude Code 运行时注入文件名（内容为全局规则发布副本），两者同体系不同语境
- 评审记录：cross-validate 第二轮实战（Kimi K3 生成 ↔ DeepSeek V4 flash 评审），对端补查 3 处己方漏审（文件头元信息、边界节「上浮」主干、observer 行路径写死）。裁决：observer 行 `D:\` 绝对路径保留（2026-08-04 路径上浮系 owner 拍板，多平台顾虑驳回）；「自动化验证可视」保留全局、措辞不动

## 2026-08-04 — init-project v1.9.0：补全局规则路径指针（Kimi 断路修复）

- [修订] `skills/init-project` v1.8.0 → v1.9.0：常驻职责新增第三条「全局规则路径指针」（owner 2026-08-04 指出）——原设计「不引导读全局文件」省上下文是有意权衡，但连 `~/AGENTS.md` 路径都没留，Kimi 会话中全局规则其余部分既未生效也无从查起（断路）；指针只给路径、按需查阅、不整体预读，原权衡不变。description、运行时识别表、验收标准、失效模式同步；HarnessOS 项目 AGENTS.md 常驻职责节同步加指针条目

## 2026-08-04 — init-project v1.8.0：废除双固定块机制，降级为普通规则条目

- [框架] `skills/init-project` v1.7.0 → v1.8.0（owner 2026-08-04 直接指示，冷冻期豁免）：废除「静默观察 + 中转站真机验证」双固定内嵌块机制，降级为项目 AGENTS.md 的两条普通规则条目——description、运行时识别表、验收标准、失效模式同步；原两节「原文照写」模板合并为一节「常驻职责条目（内容要点，措辞按项目风格自决）」。存量项目已内嵌的固定块不迁移（语义不变，仍有效）
- [修订] `skills/harness-observer` v1.5.0 → v1.6.0：「未初始化」判据从「无静默观察固定块」改为「无静默观察职责条目」（description 与显式投递条目两处）
- [修订] `skills/doc-structure` v1.4.0 → v1.5.0：AGENTS.md 模板归属句同步（固定内嵌块 → 常驻职责条目）
- [修订] HarnessOS 项目 `AGENTS.md` 同步新机制：两个固定块换成「常驻职责」一节两条普通规则条目

## 2026-08-04 — HarnessOS 自身补齐初始化：中转站真机验证固定块

- [修订] 项目 `AGENTS.md` 内嵌「中转站真机验证」固定块（init-project 标准原文照写）：owner 发现 HarnessOS 自身未按全局规则初始化——init-project v1.6.0（2026-08-01）确立双固定块标准后，16 个存量项目铺开时漏了 HarnessOS 自己，源头仓库反而缺块
