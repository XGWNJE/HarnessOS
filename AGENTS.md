...
HarnessOS · Agent 资产中枢（Agent Asset Hub，owner-declared 2026-08-04）：AI 编程助手全部可配置资产——规矩（规则）、能力（技能）、钩子（hook）、外部工具（MCP）、环境依赖（CLI/配置）——统一加工、发布、体检、交叉验证，跨环境完整重建（单一真相源在本仓库）。流水线：notes/（原料）→ 本仓库（加工）→ 发布产物。
