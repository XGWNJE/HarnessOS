...
本仓库是 Agent 通用资产的单一真相源（定位见 README.md）。流水线：notes/（原料）→ 本仓库（加工）→ 发布产物。
...
8. 责任范围只含跨项目规则、技能与 HarnessOS 自有公共 Hook；项目私有 Hook、MCP、CLI、账号、密钥和本机环境由其属主负责，本仓库不登记、不体检、不维护。

## 关键路径与命令

| 用途 | 命令 |
|---|---|
| 安装/恢复 HarnessOS 资产（写 env + 发布 + 体检） | `python scripts/install.py` |
...
| 改 `global/AGENTS.md` | 文件头版本 +1 → `sync.py` → 5 个全局发布点全同步 |
...
- 退役 skill：撤出全部发布点 + 源目录归档 `archive/skills/` + **清除发布池残留**（`~/.agents`、`~/.codex`、`~/.claude`、`~/.dsh` 四池 + `~/.config/opencode/skills` 私有池 + `dist/` 历史包）+ CHANGELOG 记废止；归档可复活。验收：`publish_skills.py --check` 无 `[残留]` 报告（脚本自动检测源外残留，来源：security-review 退役残留事件 2026-08-04）。
...
- `global/hooks/`：HarnessOS 自有公共 Hook 的源文件与注册清单
