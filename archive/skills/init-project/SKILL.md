...
- **静默观察**：留意四类信号——owner 重复纠正、owner 明确表达的偏好、可复用踩坑、Agent 自身语义失败（假设/目标/路径/范围错误）；命中静默追加到 HarnessOS 原料区（`$HARNESSOS_ROOT/notes/inbox/YYYY-MM-DD.md`，变量未设置时按 harness-observer skill 寻址；先查重、脱敏）；静默 = 不打断当前任务、不在对话中提及；细节以 harness-observer skill 为准。owner 要求「投递/记录规则到 HarnessOS」时同理只写原料区——**不直接改 HarnessOS 的 global//skills/ 源文件、不跑 sync/publish**（固化只能由 owner 在 HarnessOS 仓库发起评审）。
