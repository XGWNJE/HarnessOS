...
version: 1.8.0
name: init-project
description: 初始化或重整项目的 AGENTS.md 规则体系（规则集合）。当用户要求"初始化项目规则""为项目生成 AGENTS.md""重构/精简项目规则""新项目接入 agent 协作"时使用。目标：项目级文件以全局规则为通用规则基准（其源文件由 HarnessOS 仓库维护），只承载项目特有约束；无全局注入的运行时（Kimi Code）的两条常驻职责（静默观察、中转站真机验证）以普通规则条目写入项目 AGENTS.md（不引导读全局文件，控制上下文成本）。
---
...
Agent 启动后，通过以下环境特征识别当前运行时——仅用于决定两条常驻职责（静默观察、中转站真机验证）是否需写入项目 AGENTS.md，不做其他路由：
...
| 无上述特征，且 skill 列表来自 `~/.kimi-code/` | Kimi Code | **必须写入**两条常驻职责条目——Kimi 无全局注入机制 |
...
- **常驻职责条目到位**：项目级 AGENTS.md 含「静默观察」与「中转站真机验证」两条普通规则条目（写法同其他项目规则，要点见下节）；Kimi Code 等无全局注入的运行时因此承担对应职责；条目只写职责本身，不引导模型去读全局规则文件（避免整体读入的上下文成本）。
...
| 运行时无法识别 | 保守策略：写入两条常驻职责条目（静默观察 + 中转站真机验证，宁可多不缺漏）。除此以外本 skill 其余行为不变。 |
