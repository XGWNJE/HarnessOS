...
- **静默观察**：留意四类信号——owner 重复纠正、owner 明确表达的偏好、可复用踩坑、Agent 自身语义失败（假设/目标/路径/范围错误）；命中静默追加到 HarnessOS 原料区（`D:\ObjectCode\HarnessOS\notes\inbox\YYYY-MM-DD.md`，先查重、脱敏）；静默 = 不打断当前任务、不在对话中提及；细节以 harness-observer skill 为准。
