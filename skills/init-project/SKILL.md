...
Agent 启动后，通过以下环境特征识别当前运行时——仅用于决定固定内嵌块（静默观察块 + 中转站真机验证块）的触发策略，不做其他路由：

| 特征 | 运行时 | 固定块策略 |
|---|---|---|
| 系统提示含 `~/.claude/CLAUDE.md` 路径、存在 `Claude Code` CLI 特征 | Claude Code | **跳过**内嵌块——全局规则已注入对应职责 |
| 系统提示含 `~/.codex/` 路径、Codex 技能池特征 | Codex | **跳过**内嵌块——同上 |
| 系统提示含 `opencode` / `~/.config/opencode/` 路径 | OpenCode | **跳过**内嵌块——同上 |
| 无上述特征，且 skill 列表来自 `~/.kimi-code/` | Kimi Code | **必须内嵌**全部固定块——Kimi 无全局注入机制 |
| 无法确定 | 未知 | **内嵌**全部固定块——保守策略，宁可多一个块不缺漏 |
...
- **固定内嵌块到位**：项目级 AGENTS.md 开头含「静默观察」与「中转站真机验证」两个固定块（见下两节，原文照写）；Kimi Code 等无全局注入的运行时因此常驻承担对应职责；固定块只内嵌职责本身，不引导模型去读全局规则文件（避免整体读入的上下文成本）。
...
背景：同静默观察块——该职责写在全局规则里（「中转站真机验证兜底」），但 Kimi Code 无全局注入机制，不内嵌则该职责在 Kimi 会话中不存在。因此项目级 AGENTS.md 紧随静默观察块之后固定内嵌如下块（原文照写，不改写、不展开）：
...
| 运行时无法识别 | 保守策略：内嵌全部固定块（静默观察块 + 中转站真机验证块，宁可多不缺漏）。除此以外本 skill 其余行为不变。 |
