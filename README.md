...
除了经验型踩坑笔记，notes/ 还接受「明说规则」——owner 直接声明的意图型规则（工作习惯、工具偏好、验收标准等），加一行类型元信息：

```markdown
> 类型：明说规则
```

与经验型规则的分野：经验型规则是事后归纳，需要三门槛防误提炼；明说规则是明确意图，**豁免「重复 2 次」门槛，陈化期缩短为 ≥ 2 天**（防随口一说被永久固化），合入前须过一次**冲突检查**（对照现有 skill 与 global 规则，冲突时由 owner 裁决谁覆盖谁）。来源标注为 `owner-declared` 而非踩坑笔记；模型换代清理时，这类规则的废止标准是「owner 是否改主意」而非「场景是否失效」。

## 规则的生命周期

每条规则有三种状态变更，全部记入 CHANGELOG，条目类型标注为 **新增 / 修订 / 废止**：

1. **新增**：notes/ 中的坑同时满足三个条件才提炼——**重复出现 2 次以上**、**陈化 ≥ 14 天**（防止把一次性情绪固化成规则）、**经过至少 1 次实战验证**。合入对应 skill，版本号 +1。**明说规则例外**：豁免「重复 2 次」，陈化缩短为 **≥ 2 天**，合入前加做冲突检查（见「明说规则入口」）。
...
3. **提炼（手动，跟随评审）**：按每周评审摘要，把满足三门槛（重复 2 次以上、陈化 ≥ 14 天、实战验证 1 次）的笔记提炼为规则；明说规则按短通道（陈化 ≥ 2 天 + 冲突检查）提炼。合入对应 skill，标注来源（踩坑笔记或 owner-declared），版本号 +1，记入 CHANGELOG（新增/修订/废止）。
