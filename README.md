...
1. **新增**：notes/ 中的坑同时满足三个条件才提炼——**重复出现 2 次以上**、**陈化 ≥ 14 天**（防止把一次性情绪固化成规则）、**经过至少 1 次实战验证**。合入对应 skill，版本号 +1。
...
3. **提炼（手动，跟随评审）**：按每周评审摘要，把满足三门槛（重复 2 次以上、陈化 ≥ 14 天、实战验证 1 次）的笔记提炼为规则，合入对应 skill，标注来源，版本号 +1，记入 CHANGELOG（新增/修订/废止）。
