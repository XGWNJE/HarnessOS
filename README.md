...
首批资产类型包括 Rules、Skills、Workflows、软件与开发环境工具；未来可继续扩展硬件、服务、数据、模板、字体、模型等非软件类型。仓库只保存可审核的索引与恢复配方，不保存安装包、完整配置、账号、许可证、密钥或机器绝对路径。资产仅在用户明确要求登记、纳管或更新时写入，不主动扫描工作站或从普通对话推断个人偏好。
